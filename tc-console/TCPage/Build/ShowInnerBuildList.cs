...
namespace TC.TCPage.Build
{
    class ShowInnerBuildList
    {
        ...
        public const string BuildPattern =
            BuildIdPattern + ".*?" + EndurePattern + ".*?" + ResourcePattern;

        public const string DisabledBuildIdPattern =
            @"<li class=""build_(?<buildId>\d+) disabled"">";
        ...
        public const string BuildPrePattern =
            ...
            @"\[\[jslang\('level',(?<prdBuildLevel>\d+)\)\]\]</li>" +
            @"</ul>";

        public const string DisabledBuildPattern =
            DisabledBuildIdPattern + ".*?" + BuildPrePattern;
